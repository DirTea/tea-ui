<template>
  <div ref="rank" class="rank" :style="{width: rank_width}">
    ...
    >
      <div ref="rank_title" class="rank-title" :style="{width: title_width}">{{ item[filed_title] }}</div>
      <div class="rank-value">{{ item[filed_value] }}</div>
    </div>
  </div>
</template>
...
const startSort = () => {
  ...
  for (let i = 0; i < sortList.length - 1; i++) {
    for (let j = 0; j < sortList.length - 1 - i; j++) {
      if (props.asc ? sortList[j][props.filed_value] > sortList[j + 1][props.filed_value] : sortList[j][props.filed_value] < sortList[j + 1][props.filed_value]) {
        let temp = sortList[j];
...
const widthComputed = computed(()=>(index)=> {
  if(rank.value && rank_title.value) {
    ...
    if(props.auto_computed) {
      ...
      return titleWidth + (sortList[index][props.filed_value]/ maxValue * (rankWidth-titleWidth)) + 'px'
    } else {
      return titleWidth + (sortList[index][props.filed_value] / props.max_value * (rankWidth-titleWidth)) + 'px'
    }
  } else {
    return 'auto'
  }
